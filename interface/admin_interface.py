...
def upload_movie(user_dic,conn):
    ...
    file_path=os.path.join(setting.BASE_MOVIE_LIST,file_name)
    ...
    movie=models.Movie(
                       ...
                       is_delete=0,
                       creame_time=common.get_time(),
                       user_id=user_dic['user_id'],
                       file_md5=user_dic['file_md5'],
                       )
    movie.save()
    back_dir={'flag':True,'msg':'上传成功'}
    common.send_back(back_dir,conn)
...
def check_movie(user_dic,conn):
    ...
    if movie:
        back_dic={'flag':False,'msg':'此电影已经存在'}
    else:
        back_dic={'flag':True}
    common.send_back(back_dic,conn)
